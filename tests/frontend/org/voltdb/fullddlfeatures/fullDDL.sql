...
-- not yet hsql232 -- not supporting CASCADE option -- is this important?
...
-- not yet hsql232 -- strange error on each of these lines "VoltDB does not support sequences or subqueries"
-- not yet hsql232 -- CREATE PROCEDURE PROC_USES_NULL AS SELECT DECODE(STR, NULL, 'NULLISH', STR) FROM T55;
-- not yet hsql232.
CREATE PROCEDURE PROC_USES_TO_TIMESTAMP AS SELECT TO_TIMESTAMP(SECOND, BIG) FROM T55;
-- not yet hsql232 -- fails assert after strange error "VoltDB does not support sequences or subqueries"
-- not yet hsql232 -- CREATE VIEW V55_USES_NULL AS
-- not yet hsql232 --     SELECT   DECODE(STR, NULL, 'NULLISH', STR), COUNT(*), SUM(BIG)
-- not yet hsql232 --     FROM T55 
-- not yet hsql232 --     GROUP BY DECODE(STR, NULL, 'NULLISH', STR);
-- not yet hsql232.
...
-- hsql232 ENG-8331 issue with DECODE: CREATE INDEX ENG_8168_INDEX_USES_DECODE ON T55 (DECODE(STR, 'X', 'Y', 'Z'));
-- ENG-7840 Make sure that a NULL constant survives DDL roundtripping.
-- hsql232 ENG-8331 issue with DECODE: CREATE INDEX ENG_7840_INDEX_USES_DECODE_AND_NULL ON T55 (DECODE(STR, NULL, 'NULLISH', STR));
...
CREATE INDEX ENG_8168_INDEX_USES_SINCE_EPOCH_SECOND ON T55 (SINCE_EPOCH(SECOND, TS)/60);
